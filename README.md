...
Blueprint (Free web service plan): connect the GitHub repo in Render's "New Blueprint"
flow and it builds with `npm install` / runs `npm start` automatically. `JWT_SECRET` is
auto-generated by Render; `DB_FILE` defaults to `./data/rentlogg.db`.

**Currently on ephemeral storage** — the Free plan has no persistent disk, so
`data/rentlogg.db` and `uploads/` reset on every redeploy (they survive normal restarts
and idle spin-down, just not a new deploy). After each deploy, reseed via the Render
Shell tab:
...
To make data durable across deploys, upgrade the service to a paid instance (Starter,
~$7/mo) and attach a Render persistent disk mounted at a path covered by `DB_FILE` and
`UPLOADS_DIR` (both are already configurable via env vars for this reason — see
[src/server.js](src/server.js) and [src/db.js](src/db.js)).
